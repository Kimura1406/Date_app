...
func NewRouter(
	...
	mux.HandleFunc("POST /api/v1/flowers/{id}/acquire", withAuth(tokenManager, flowerHandler.AcquireFlower))
	mux.HandleFunc("GET /api/v1/me/flowers", withAuth(tokenManager, flowerHandler.ListOwnedFlowers))
	mux.HandleFunc("GET /api/v1/banners", bannerHandler.ListPublicBanners)
	...
	mux.HandleFunc("GET /api/v1/me/notifications", withAuth(tokenManager, userHandler.ListNotifications))
	...
	mux.HandleFunc("POST /api/v1/users/{id}/profile-views", withAuth(tokenManager, userHandler.RecordProfileView))
	...
	mux.HandleFunc("GET /api/v1/me/likes/users", withAuth(tokenManager, userHandler.ListUsersWhoLikedMe))
	mux.HandleFunc("POST /api/v1/users/{id}/block", withAuth(tokenManager, userHandler.BlockUser))
	mux.HandleFunc("GET /api/v1/me/blocked-users", withAuth(tokenManager, userHandler.ListBlockedUsers))
	mux.HandleFunc("POST /api/v1/users/{id}/report", withAuth(tokenManager, userHandler.ReportUser))
